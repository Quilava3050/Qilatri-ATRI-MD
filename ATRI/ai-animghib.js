...
let handler = async (m, { conn, command }) => {
  try {
    ...
    if (!mime.startsWith('image/')) return m.reply('❌ Reply gambar yang ingin diubah!')
 
    m.reply('🎨 Sedang memproses gambar...')
 
    const media = await q.download()
    const tempPath = join(process.cwd(), 'tmp', `${Date.now()}.jpg`)
    ...
    // Perbaikan pengiriman gambar
    await conn.sendMessage(m.chat, { 
      image: { url: result.imageUrl },
      caption: `✨ Berhasil mengubah gambar ke style ${useGhibli ? 'Ghibli' : 'Anime'}!`,
      mimetype: 'image/jpeg'
    }, { quoted: m })
...
handler.tags = ['ai']
handler.command = /^(toanime|toghibli)$/i
handler.limit = true
