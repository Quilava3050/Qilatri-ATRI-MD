...
let handler = async (m, { conn, usedPrefix, command }) => {
  try {
    ...
    m.reply('💡 Mengubah gambarmu dengan efek Luminare... tunggu sebentar ya!')

    let media = await q.download()
    let url = await uploadImage(media)
    if (!url) throw '❌ Gagal mengunggah gambar untuk diproses.'

    let api = `https://api.elrayyxml.web.id/api/ephoto/luminare?url=${encodeURIComponent(url)}`
    ...
    await conn.sendMessage(m.chat, {
      image: buffer,
      caption: '✨ Selesai — ini hasil efek Luminare dari gambarmu.'
    }, { quoted: m })
...
handler.help = ['toluminare']
handler.tags = ['ai']
handler.command = /^toluminare$/i
handler.limit = true
handler.daftar = true
