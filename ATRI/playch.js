...
let handler = async (m, { conn, text, command }) => {
  ...
  try {
    ...
    // ambil thumbnail acak dari 2 URL
    const thumbs = [
      'https://idweb.tech/api/preview.php?file=qzhpumzc.png',
      'https://idweb.tech/api/preview.php?file=gmcxvot0.png'
    ]
    const customThumb = thumbs[Math.floor(Math.random() * thumbs.length)]
...
${global.wm}
    ...
    // kirim tampilan dengan custom thumbnail
    ...
    // konversi audio ke VN tanpa thumb
    const id = crypto.randomBytes(6).toString('hex')
    const tmpDir = os.tmpdir()
    const tmpMp3 = path.join(tmpDir, `yt-${id}.mp3`)
    const tmpOgg = path.join(tmpDir, `yt-${id}.ogg`)

    await downloadToFile(dl.result.download, tmpMp3)
    await convertToOggOpus(tmpMp3, tmpOgg)
    const vnBuffer = await fs.promises.readFile(tmpOgg)

    await conn.sendMessage(m.chat, {
      audio: vnBuffer,
      mimetype: 'audio/ogg; codecs=opus',
      ptt: true,
      fileName: `${sanitizeFilename(apiTitle)}.opus`
    }, { quoted: m })

    try { await fs.promises.unlink(tmpMp3) } catch (e) {}
    try { await fs.promises.unlink(tmpOgg) } catch (e) {}
...
async function downloadToFile(url, filepath) {
  ...
  const res = await axios({
    ...
    url,
    responseType: 'stream',
    headers: { 'User-Agent': 'Mozilla/5.0 (Node) Downloader' },
    timeout: 60000
  })
...
async function convertToOggOpus(inputPath, outputPath) {
  return new Promise((resolve, reject) => {
    ...
    ff.on('error', err => reject(new Error('ffmpeg execution error: ' + err.message)))
    ff.on('close', code => {
...
const savetube = {
  ...
  download: async (link, format) => {
    if (!link) return { status: false, code: 400, error: "Link kosong" }
