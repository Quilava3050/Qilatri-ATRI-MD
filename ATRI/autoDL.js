...
// Cache untuk mencegah duplikasi download
...
handler.before = async function (m, { conn, isPrems }) {
    ...
    const cacheKey = `${m.chat}_${text}`;
    const now = Date.now();
    
    if (downloadCache.has(cacheKey)) {
        const lastDownload = downloadCache.get(cacheKey);
        if (now - lastDownload < CACHE_DURATION) {
            return; // Skip jika masih dalam periode cache
        }
    }
...
async function _tiktok(link, m) {
    try {
        if (global.db.data.users[m.sender].limit > 0) {
            const response = await fetch(`https://api.botcahx.eu.org/api/download/tiktok?url=${link}&apikey=${global.btc}`);
            ...
            if (data.result.video.length > 1) {
                // Kirim multiple video untuk slide/carousel
                for (let i = 0; i < data.result.video.length; i++) {
                    await conn.sendFile(m.chat, data.result.video[i], null, `🍟 *Fetching* : ${(new Date() - old) * 1} ms (${i + 1}/${data.result.video.length})`, m);
                    if (i < data.result.video.length - 1) await _sleep(2000);
                }
            } else {
                // Kirim single video
...
async function _douyin(link, m) {
    try {
        if (global.db.data.users[m.sender].limit > 0) {
            let response = await fetch(`https://api.botcahx.eu.org/api/download/douyin?url=${link}&apikey=${global.btc}`);
            let data = await response.json();
            
            // Cek apakah ada video
            if (!data.result.video || data.result.video.length === 0) {
                // Coba untuk slide/images
                response = await fetch(`https://api.botcahx.eu.org/api/download/douyinslide?url=${link}&apikey=${global.btc}`);
                data = await response.json();
                
                if (data.result.images && data.result.images.length > 0) {
                    global.db.data.users[m.sender].limit -= 1;
                    for (let i = 0; i < data.result.images.length; i++) {
                        await conn.sendFile(m.chat, data.result.images[i], null, `🍟 *Fetching* : ${(new Date() - old) * 1} ms (${i + 1}/${data.result.images.length})`, m);
                        if (i < data.result.images.length - 1) await _sleep(2000);
                    }
                    conn.sendMessage(m.chat, { react: { text: "✅", key: m.key } });
                    return;
                }
...
async function _instagram(link, m) {
    try {
        if (global.db.data.users[m.sender].limit > 0) {
            const response = await fetch(`https://api.botcahx.eu.org/api/dowloader/igdowloader?url=${link}&apikey=${global.btc}`);
            const res = await response.json();
            const limitnya = 3;
            for (let i = 0; i < Math.min(limitnya, res.result.length); i++) {
                await _sleep(3000);
                await conn.sendFile(m.chat, res.result[i].url, null, `🍟 *Fetching* : ${(new Date() - old) * 1} ms`, m);
            }
            global.db.data.users[m.sender].limit -= 1;
            conn.sendMessage(m.chat, { react: { text: "✅", key: m.key } });
        } else {
...
async function _facebook(link, m) {
    try {
        if (global.db.data.users[m.sender].limit > 0) {
            const response = await fetch(`https://api.botcahx.eu.org/api/dowloader/fbdown3?url=${link}&apikey=${global.btc}`);
            let json = await response.json();
            let urls = json.result.url.urls;
            if (Array.isArray(urls) && urls.some((url) => url.sd)) {
                global.db.data.users[m.sender].limit -= 1;
                let videoUrl = urls.find((url) => url.sd).sd;
                await conn.sendFile(m.chat, videoUrl, "fb.mp4", `🍟 *Fetching* : ${(new Date() - old) * 1} ms`, m);
                conn.sendMessage(m.chat, { react: { text: "✅", key: m.key } });
            } else {
                conn.sendMessage(m.chat, { react: { text: "❌", key: m.key } });
...
async function _youtube(link, m) {
    try {
        if (global.db.data.users[m.sender].limit > 0) {
            const response = await fetch(`https://api.botcahx.eu.org/api/dowloader/yt?url=${link}&apikey=${global.btc}`);
            const result = await response.json();
            
            if (result.status && result.result && result.result.mp4) {
                global.db.data.users[m.sender].limit -= 1;
                
                // Kirim hanya video dengan caption info
                await conn.sendMessage(
                    m.chat,
                    {
                        video: {
...
async function _twitter(url, m) {
    try {
        if (global.db.data.users[m.sender].limit > 0) {
            const api = await fetch(`https://api.botcahx.eu.org/api/download/twitter2?url=${url}&apikey=${global.btc}`);
            ...
            if (res.result && res.result.mediaURLs) {
                ...
                for (const url of mediaURLs) {
                    ...
                    await _sleep(3000);
                    await conn.sendFile(m.chat, buffer, null, `🍟 *Fetching* : ${(new Date() - old) * 1} ms`, m);
                }
                conn.sendMessage(m.chat, { react: { text: "✅", key: m.key } });
            } else {
                conn.sendMessage(m.chat, { react: { text: "❌", key: m.key } });
...
async function _threads(url, m) {
    try {
        if (global.db.data.users[m.sender].limit > 0) {
            const apiResponse = await fetch(`https://api.botcahx.eu.org/api/download/threads?url=${url}&apikey=${global.btc}`);
...
async function _capcut(url, m) {
    try {
        if (global.db.data.users[m.sender].limit > 0) {
            const response = await fetch(`https://api.botcahx.eu.org/api/dowloader/capcut?url=${url}&apikey=${global.btc}`);
            const res = await response.json();
...
async function _snackvideo(url, m) {
    try {
        if (global.db.data.users[m.sender].limit > 0) {
            const api = await fetch(`https://api.botcahx.eu.org/api/download/snackvideo?url=${url}&apikey=${global.btc}`);
            const res = await api.json();
...
async function _pindl(link, m) {
    try {
        if (global.db.data.users[m.sender].limit > 0) {
            const api = await fetch(`https://api.botcahx.eu.org/api/download/pinterest?url=${link}&apikey=${global.btc}`);
            const res = await api.json();
...
async function _xiaohongshu(url, m) {
    try {
        if (global.db.data.users[m.sender].limit > 0) {
            let res = await axios.get(`https://api.botcahx.eu.org/api/download/rednote?url=${url}&apikey=${global.btc}`);
