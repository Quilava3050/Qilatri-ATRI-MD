...
async function createSticker(img, url, packName, authorName, quality) {
	let stickerMetadata = {
		type: 'full',
		pack: packName || global.stickpack,
		author: authorName || global.stickauth,
		quality
	}
...
async function mp4ToWebp(file, stickerMetadata) {
	if (stickerMetadata) {
		if (!stickerMetadata.pack) stickerMetadata.pack = global.stickpack
		if (!stickerMetadata.author) stickerMetadata.author = global.stickauth
		if (!stickerMetadata.crop) stickerMetadata.crop = false
	} else if (!stickerMetadata) {
		stickerMetadata = { pack: global.stickpack, author: global.stickauth, crop: false }
	}
