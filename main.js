...
if (!conn.authState.creds.registered) {
    ...
    do {
        ...
        PHONENUMBER_MCC && Object.keys(PHONENUMBER_MCC).some((v) => phoneNumber.startsWith(v))
    );
    rl.close();
...
function clearTmp() {
    ...
    tmpPaths.forEach((dirname) => {
        ...
        if (!existsSync(dirname)) {
            try {
                ...
                console.error('Gagal membuat folder tmp:', err);
                return;
            }
        }
        ...
        try {
            ...
            console.error(`Gagal membaca folder: ${dirname}`, err);
        }
    });
    ...
    return filename.map((file) => {
        try {
            ...
            console.error('Gagal menghapus file tmp:', err);
        }
        return false;
    });
}
...
async function connectionUpdate(update) {
  ...
  if (global.db.data == null) loadDatabase()
    if (connection === "open") {
        ...
        const infoMsg = `*Lapor! 🫡*\n\n_Bot berhasil terhubung ke server dengan baik_
        `;
  ...
  if (code === DisconnectReason.loggedOut) {
    console.log(chalk.red('❌ Session logged out. Silakan hapus folder sessions dan scan ulang pairing code.'))
  } else {
...
global.reloadHandler = async function(restatConn) {
  ...
  if (!isInit) {
   ...
   conn.ev.off('message.delete', conn.onDelete)
    ...
    // Use placeholders expected by handler: @user and @desc
...
const pluginFolder = global.__dirname(join(__dirname, './ATRI/index'));
...
async function filesInit() {
    for (const filename of readdirSync(pluginFolder).filter(pluginFilter)) {
        try {
            ...
            global.ATRI[filename] = module.default || module;
        } catch (e) {
            conn.logger.error(e);
            delete global.ATRI[filename];
        }
    }
}
filesInit().then((_) => Object.keys(global.ATRI)).catch(console.error);

global.reload = async (_ev, filename) => {
    if (pluginFilter(filename)) {
        const dir = global.__filename(join(pluginFolder, filename), true);
        if (filename in global.ATRI) {
            if (existsSync(dir)) conn.logger.info(` Updated Plugin - '${filename}'`);
            else {
                conn.logger.warn(`Deleted Plugin - '${filename}'`);
                return delete global.ATRI[filename];
            }
        ...
        if (err) conn.logger.error(`syntax error while loading '${filename}'\n${format(err)}`);
            ...
            try {
                ...
                global.ATRI = Object.fromEntries(Object.entries(global.ATRI).sort(([a], [b]) => a.localeCompare(b)));
            }
        }
    }
};
