...
const spotifydl = async (url) => {
    try {
        ...
        if (!yanzzData?.result?.gid || !yanzzData?.result?.id) {
            throw new Error('Gagal mengambil data awal dari fabdl!');
        }
        ...
        if (!yanzData?.result?.download_url) {
            throw new Error('Gagal mengambil download URL dari fabdl!');
        }
