...
* bálzz 
*//*
export async function sendContact(jid, contact, quoted, options) {
    let list = [];
    contact.map(v => list.push({
        displayName: v.name,
        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:${v.name}\nORG:${info && info.org ? info.org : 'balzz'}\nTEL;type=CELL;type=VOICE;waid=${v.number}:${PhoneNumber('+' + v.number).getNumber('international')}\nEMAIL;type=Email:${info && info.email ? info.email : 'pa424013@gmail.com'}\nURL;type=Website:${info && info.website ? info.website : 'https://api.balzz.my.id'}\nADR;type=Location:;;${info && info.location ? info.location : 'isekai'};;\nOther:${v.about}\nEND:VCARD`
    }));
...
* balzz
* dims
* footer with media/png/jpg
*/
sendFooterWithMedia: {
async value(jid, text, footer, media, quoted, options) {
...
**/
kirimBetonDenganImage: {
async value(jid, text, footer, media, butText, butCmd, quoted, options) {
  ...
  viewOnceMessage: {
    message: {
        ...
        interactiveMessage: proto.Message.InteractiveMessage.create({
          ...
          newsletterJid: '120363193509141242@newsletter',
          serverMessageId: '482EF217AF2B029F25D5689CA39A1334',
          newsletterName: 'waSocket 2023 by Amirul Dev'*/
          },
          externalAdReply: {
...
`.trim()
        ...
        reply: {
            ...
            value(jid, text = '', quoted, options = {}, smlcap = { smlcap : true }) {
                ...
                if (global.db.data.settings[conn.user.jid].adReply) {
                    ...
                    return Buffer.isBuffer(text) ? conn.sendFile(jid, text, 'file', '', quoted, false, options) : conn.adReply(jid, text, global.config.watermark, "", fs.readFileSync('./media/' + thumb.getRandom() + '.jpg'), false, quoted, false, false, smlcap)
                } else {
                    return Buffer.isBuffer(text) ? conn.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, { text: isSmlcap ? conn.smlcap(text, smlcap.except ? smlcap.except : false) : text, ...options }, { quoted, ...options, ephemeralExpiration: ephemeral })
                }
            }
        },
        adReply: {
            async value(jid, text, title = '', body = '', buffer, source = '', quoted, large = true, options = {}, smlcap = { smlcap: true }) {
                ...
                return conn.sendMessage(jid, { text: isSmlcap ? conn.smlcap(text, smlcap.except ? smlcap.except : false) : text, 
                    contextInfo: {
        ...
        sendButton: {
            async value(jid, title, body, footer, button = [], m, options = {}, smlcap = { smlcap: true }) {
                let isSmlcap = global.db.data.settings[conn.user.jid].smlcap && smlcap.smlcap
                let ephemeral = conn.chats[jid]?.metadata?.ephemeralDuration || conn.chats[jid]?.ephemeralDuration || false
                ...
                let msg = generateWAMessageFromContent(m.chat, {
                    viewOnceMessage: {
                        message: {
                            ...
                            interactiveMessage: proto.Message.InteractiveMessage.create({
                                body: proto.Message.InteractiveMessage.Body.create({
                                    text: isSmlcap ? conn.smlcap(body, smlcap.except ? smlcap.except : false) : body,
                                }),
                                footer: proto.Message.InteractiveMessage.Footer.create({
                                    text: isSmlcap ? conn.smlcap(footer, smlcap.except ? smlcap.except : false) : footer,
                                }),
                                header: proto.Message.InteractiveMessage.Header.create({
                                    title: isSmlcap ? conn.smlcap(title, smlcap.except ? smlcap.except : false) : title,
                                    subtitle: "",
                                    hasMediaAttachment: false,
                                   // ...media
...
END:VCARD`.trim()
       ...
       /*sendButton: {
        ...
        sendButtonImg: {
            async value(jid, image, title, body, footer, button = [], m, options = {}, smlcap = { smlcap: true }) {
                let isSmlcap = global.db.data.settings[conn.user.jid].smlcap && smlcap.smlcap
                let ephemeral = conn.chats[jid]?.metadata?.ephemeralDuration || conn.chats[jid]?.ephemeralDuration || false
        ...
        sendButtonVid: {
            async value(jid, video, title, body, footer, button = [], m, options = {}, smlcap = { smlcap: true }) {
                let isSmlcap = global.db.data.settings[conn.user.jid].smlcap && smlcap.smlcap
                let ephemeral = conn.chats[jid]?.metadata?.ephemeralDuration || conn.chats[jid]?.ephemeralDuration || false

                const media = await prepareWAMessageMedia(
                    { video: (await conn.getFile(video)).data },
                    { upload: conn.waUploadToServer }
                )
