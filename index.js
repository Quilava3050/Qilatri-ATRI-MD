...
say('ATRI MD', {
  ...
  align: 'center',
  colors: ['white'] })
say(`Simple Whatsapp Bot By Dhieka`, { 
  font: 'console',
  align: 'center',
  colors: ['green'] })
})
...
async function start(file) {
  ...
  const ATRIFolder = path.join(path.dirname(currentFilePath), 'ATRI');

  fs.readdir(ATRIFolder, async (err, files) => {
    if (err) {
      console.error(chalk.red(`Folder ATRI Error: ${err}`));
      return;
    }
    console.log(chalk.yellow(`Ditemukan ${files.length} File Di Folder ATRI`));
  ...
  console.log(chalk.yellow(`Script by Dhieka`));
  ...
  try {
    ...
    console.log(chalk.cyan(`Author: Dhieka`));
  } catch (err) {
    console.error(chalk.red(`Tidak Bisa Membaca File package.json: ${err}`));
  }

  const totalFoldersAndFiles = await getTotalFoldersAndFiles(ATRIFolder);
  console.log(chalk.blue.bold(`\nTotal Folder Dan Files Di "ATRI"`));
